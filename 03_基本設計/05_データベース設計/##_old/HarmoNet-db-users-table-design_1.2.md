# HarmoNet DB 設計書: public.users テーブル定義 v1.2
...
4. メールアドレスとニックネームは、ログイン運用とトラブルシュートを簡潔にするため、テーブル全体でユニークとする。
5. 作成・更新時刻は全テーブル共通方針に従い、`created_at` / `updated_at` の 2 カラムで管理する。
...
| display_name   | varchar     | 32  | NOT NULL | アプリ上の表示名（ニックネーム）。例: 「山田家」「山田太郎」。UK2 で一意。        |
...
| phone_number   | varchar     | 16  | NULL     | 電話番号。国内利用前提の任意項目。形式は固定しない。                      |
