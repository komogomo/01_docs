...
* Prisma Migrate の migration ファイルは使用しない。
  ...
  * スキーマ変更は `schema.prisma` を直接編集し、`npx prisma db push` で DB に反映する。
...
| 姓       | 必須 | 255文字以内                    | `users.last_name`                 |
| 名       | 必須 | 255文字以内                    | `users.first_name`                |
| 姓ふりがな   | 任意 | 255文字以内（ひらがな前提、詳細は詳細設計で定義） | `users.last_name_kana`            |
| 名ふりがな   | 任意 | 255文字以内（ひらがな前提、詳細は詳細設計で定義） | `users.first_name_kana`           |
...
* 姓ふりがな / 名ふりがな
  ...
  * 任意
  * 255文字以内
  * 入力がある場合はひらがなのみ（具体的なチェックロジックは詳細設計で定義）
* 言語
...
3. `public.users` の upsert
   ...
   * `email`, `display_name`, `language` に加え、以下を保存/更新
...
| Version | 日付         | 変更内容                                        |
| ------- | ---------- | ------------------------------------------- |
| 1.2     | 2025-12-06 | `users` スキーマ変更に合わせ、ユーザ登録項目を姓・名＋ふりがなに分割して定義。 |
