# ==========================================================
# HarmoNet Git Utility v4.6 (Console / Stable)
# Author : Tachikoma + Claude
# Purpose: Safe Git operations with menu UI for HarmoNet Phase9
# Changelog v4.6:
#   - Fixed: Graphics driver conflict during multi-line input
#   - Changed Read-Host to Console.ReadLine() with error handling
#   - Added small delay between inputs to prevent driver issues
#   - Inherits all v4.5 improvements
# ==========================================================
...
function Read-MultiLineInput {
    ...
    while ($true) {
        try {
            Write-Host "[$lineNum] " -NoNewline
            
            # Read-Host の代わりに Console.ReadLine() を使用
            # グラフィックスドライバとの競合を回避
            ...
            Write-Host "⚠ 入力エラーが発生しました。再試行します..." -ForegroundColor Yellow
...
do {
    Clear-Host
    Write-Host "========================================="
    Write-Host "   HarmoNet Git Utility v4.6 (Console)"
    Write-Host "========================================="
    ...
    do {
        ...
        switch ($m) {
            ...
            "3" {
                ...
                # グラフィックスドライバ対策版の入力関数を使用
                $msg = Read-MultiLineInput "メッセージを入力してください:"
