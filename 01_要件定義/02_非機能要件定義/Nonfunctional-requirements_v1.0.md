...
本書は HarmoNet システム全体に適用される **非機能要件（NFR）** を定義する。システム品質、運用性、保守性、安全性、可用性など、機能要件では表現できない横断的要件を明確化することで、基本設計・詳細設計・実装および運用全体の基準とする。

本書は認証方式（MagicLink / Passkey）に限定されず、**HarmoNet 全体に対する共通品質要件**を扱う。
...
個別方式（MagicLink, Passkey）の資料、基本設計、詳細設計の上位に位置する。

後続工程で要件に変更が必要な場合は、必ず TKD の承認を得て版管理を行う。
