...
**技術スタック準拠:** HarmoNet Technical Stack Definition v4.1（Next.js 16.1 / React 19.0 / Supabase Cloud / Corbado SDK公式構成 / TailwindCSS 4.0 / Windsurf AI連携）
...
HarmoNetは、**Next.js 16.1 + React 19.0 + Supabase Cloud + Corbado SDK + TailwindCSS 4.0 + Prisma ORM** を中核とする、AI統合型マルチテナントSaaS基盤である。
...
1. **技術統一:** Supabase Cloud + Next.js App Router構成を中核とする。
...
| 区分          | 内容                                                           |
| ----------- | ------------------------------------------------------------ |
| **フロントエンド** | Next.js 16.1 (App Router) + React 19.0 + TailwindCSS 4.0     |
| **バックエンド**  | Supabase Cloud（Edge Function + PostgreSQL 17）                |
| **認証**      | Supabase MagicLink + Corbado Passkey（short_session Cookie構成） |
| **翻訳・音声**   | StaticI18nProvider + Google Translate v3 + VOICEVOX API      |
| **通知・ログ**   | Supabase Realtime + Sentry + Audit Log（RLS適用）                |
| **AI連携**    | タチコマ（要件統合）＋Gemini（品質検証）＋Windsurf（実装生成）                       |
| **ホスティング**  | Vercel + Supabase Cloud（Pro Plan基準）                          |
...
| 実装・テスト自動化  | Windsurf     | Next.js + Supabase 実装／Jest／CodeAgent_Report生成 |
...
| ドキュメント名                                                    | 概要                               |
| ---------------------------------------------------------- | -------------------------------- |
| **harmonet-technical-stack-definition_v4.1.md**            | 技術基盤（Supabase Cloud + Corbado構成） |
...
| **functional-requirements-ch02-system-overview_v1.4.md**   | システム全体構成・AI体制定義（本書連携）            |
| **functional-requirements-ch04-nonfunctional-req_v1.4.md** | 非機能要件（セキュリティ／性能／可用性）             |
| **functional-requirements-ch05-config-env_v1.5.md**        | 外部API・認証・環境変数定義                  |
...
本要件定義書は HarmoNet の**正式運用構成（v4.1）**に基づき作成されている。
