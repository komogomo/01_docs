# HarmoNet 機能要件定義書 第5章（外部API・認証・環境構成要件） v1.6

**技術スタック準拠:** HarmoNet Technical Stack Definition v4.3（Supabase + Corbado SDK公式構成 + Next.js 16.x + Docker開発環境）
**Document ID:** HNM-REQ-CH05-20251117
**Version:** 1.6
...
Next.js 16.x + Supabase v2.43 + Corbado SDK（React + Node構成）を前提とし、開発環境（Docker Desktop）と本番環境（Supabase Cloud）のクラウド実運用を見据えたセキュアな構成要件を示す。
...
HarmoNetは **パスワードレス構成** を採用し、以下の二方式を独立したUIボタンとして提供する（v4.3準拠）。
...
### 3.2 Corbado構成（v4.3公式構成）

| 層 | ライブラリ | 役割 |
|----|----------|------|
| フロント | `@corbado/react` (v2.x) | `<CorbadoProvider />` + `<Auth />` によりPasskey認証を実施 |
| サーバ | `@corbado/node` (v2.x) | `/api/auth/corbado` にてJWT検証を実行 |
...
## 第5章　開発環境構成（Docker Desktop）

| 項目 | 内容 |
|------|------|
| 基盤 | Docker Desktop + Supabase CLI |
| データベース | PostgreSQL 17 (Dockerコンテナ) |
| メールテスト | Mailpit（ローカルSMTPサーバー、http://localhost:8025） |
| Edge Function | Supabase CLI内蔵のDeno Runtime |
| 起動コマンド | `supabase start` |
| 停止コマンド | `supabase stop` |
| マイグレーション | `supabase db push` / `supabase db reset` |

---

## 第6章　本番環境構成（Supabase Cloud）
...
| バックアップ | Supabase自動スナップショット（1日1回、保持7日間） |
...
| 拡張性 | Supabase Edge分散構成・Corbado Node統合 | 将来対応設計 |
...
| **2025-11-17** | **1.6** | **v4.3完全対応：Next.js 16.x、shadcn/ui追加、Docker開発環境詳細化、MagicLink/Passkey独立方式** | **Tachikoma / TKD** |

---

**Directory:** `/01_docs/01_requirements/01_機能要件定義/`
**Reviewer:** TKD
**Status:** ✅ HarmoNet正式要件（技術スタックv4.3整合）
