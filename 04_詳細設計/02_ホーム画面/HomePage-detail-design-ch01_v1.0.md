...
2. **よく使う機能への入口**
   ...
   * 掲示板（ALL/NOTICE/RULES）、施設予約、運用ルール、通知設定など、
...
* コンテンツ領域のレイアウト
  ...
  * 「最新のお知らせ」＋「機能タイル 2×3」の縦並び構成
* お知らせセクションの UI・挙動

  * 表示件数（1〜3件）の制御（MVPでは 2件固定として実装開始）
  * 0件時メッセージ「現在表示するお知らせはありません。」の表示
  * カードタップ時の遷移（掲示板詳細への Link 想定。実装時期は別途）
* 機能タイルの UI・挙動

  * 上段：お知らせ／掲示板／施設予約
  * 下段：運用ルール／通知設定／ダミー
  * `isEnabled` フラグによるリンク有効／無効の制御
