# SA-01 システム管理者向けテナント管理・テナント管理者管理 詳細設計書 v1.4
...
* SysAdmin テナント管理・テナント管理者管理 基本設計書 v1.3
* テナント管理者用ユーザ登録画面 詳細設計書 ch01 v1.0（/t-admin/users）
* schema.prisma（最新版: users.last_name / fast_name / last_name_kana / fast_name_kana 仕様）
...
これらはすべて **SA-01 画面内のセクションとして統合**し、画面遷移を発生させない。実装済み UI（テナント詳細＋テナント管理者管理＋テナント一覧が 1 画面に収まっている構成）と一致させる。
...
* DB スキーマは最新版 `schema.prisma` を正とし、本機能で直接利用するテーブルは以下とする。
...
* `users` の氏名情報は v1.4 から以下の 4 カラムで管理し、`full_name` は使用しない。

  * `last_name`（姓）
  * `fast_name`（名）
  * `last_name_kana`（姓ふりがな）
  * `fast_name_kana`（名ふりがな）
...
* 上部に AppHeader（ロゴ、言語切替、通知アイコンなど）

* コンテンツエリアは次の 3 セクションから構成される。

  1. **SEC-01 テナント詳細パネル**（画面上部）
  2. **SEC-02 テナント管理者管理パネル**（中段）
  3. **SEC-03 テナント一覧テーブル**（下段）

* テナント一覧で行選択を行うと、SEC-01 / SEC-02 の内容が選択テナントに切り替わる。

---

### 2.2 SEC-01 テナント詳細パネル

（※ ここは v1.3 から変更なし・既存仕様を維持）
...
* 管理者一覧テーブル
  ...
  * 列
    ...
    * メールアドレス
    * 表示名（ニックネーム）
    * 氏名（表示上は `last_name` + `fast_name` を連結して表示）
    * 最終ログイン（任意、取得できる場合のみ）
    * 操作（編集 / 管理者解除）
...
* ボタン
  ...
  * 「管理者ユーザ登録」または「管理者ユーザ更新」（フォームモードに応じてラベル切替）

#### 2.3.2 操作仕様

* 管理者一覧行の「編集」

  * 下部フォームに選択ユーザの **メールアドレス / 表示名 / 姓 / 名 / 姓ふりがな / 名ふりがな** を読み込み、編集モードにする。
  * メールアドレスは Supabase Auth の ID として利用するため編集不可。
  * 「管理者ユーザ更新」ボタンで `users` レコード（`display_name` / `last_name` / `fast_name` / `last_name_kana` / `fast_name_kana`）を更新。
...
* 管理者登録フォーム
  ...
  * メールアドレス / 表示名 / 姓 / 名 は必須。
  * 姓ふりがな / 名ふりがな は任意（将来の検索や表示用途を考慮して保持）。
  * 「管理者ユーザ登録」ボタンで新規登録処理を実行（4.5 参照）。

---

### 2.4 SEC-03 テナント一覧テーブル
...
* 行選択時
  ...
  * `selectedTenantId` を対象行のテナントIDに更新し、SEC-01 / SEC-02 の表示内容を更新する。
...
* 内部セクションコンポーネント（例）
  ...
  * `TenantDetailPanel`（SEC-01）
  * `TenantAdminPanel`（SEC-02、一覧＋登録フォーム）
  * `TenantListPanel`（SEC-03）
* 共通レイアウト
...
await prisma.users.create({
  data: {
    ...
    last_name_kana,
    fast_name_kana,
    group_code: "SYSADMIN", // system_admin 用ダミー値（詳細は DB 設計書で定義）
    residence_code: "SYSADMIN", // 同上
    // その他必須カラムも適宜設定
  },
});
```
...
* 管理者一覧からユーザを選択し、フォームで表示名・氏名・ふりがなを編集する。
...
model users {
  ...
  fast_name_kana String?     // 名ふりがな
  group_code     String
  residence_code String
  // ... そのほかのカラムは DB 設計書を正とする
}
...
* SA-UT-02: テナント行選択で SEC-01 / SEC-02 が選択テナントの情報に更新されること。
...
* SA-UT-13: 管理者登録・編集時に、`users.last_name` / `fast_name` / `last_name_kana` / `fast_name_kana` に入力内容が正しく保存されること。
* SA-UT-14: 管理者一覧の「氏名」列が、`last_name` + `fast_name` の連結結果として表示されること。
...
| ------- | ---------- | ----------- | ----------------------------------------------------------- |
| 1.4     | 2025-12-06 | Tachikoma   | 管理者氏名項目を fast_name / last_name 系カラムに分割。UI と更新ロジック、データ仕様を修正。 |
