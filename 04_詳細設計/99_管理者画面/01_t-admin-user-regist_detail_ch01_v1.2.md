# テナント管理者用ユーザ登録画面 詳細設計書 ch01（画面＋ロジック統合版） v1.2
...
- **すべての API で、共通認証ヘルパー `getTenantAdminApiContext` を利用して権限チェックを行う。**

```ts
import { getTenantAdminApiContext } from "@/src/lib/auth/tenantAdminAuth";

// Route Handler 内での利用例
const { adminClient, user, tenantId } = await getTenantAdminApiContext();
...
1. `getTenantAdminApiContext` によりセッション・`tenant_id` を取得。
...
1. `getTenantAdminApiContext` によりセッション・`tenant_id` を取得。
2. 入力バリデーション（必須、一意性チェックなど）を実施。
3. `users` テーブルでメールアドレス重複を確認。存在する場合は 409(CONFLICT)。
4. `users` テーブルでニックネーム重複を確認。存在する場合は 409(CONFLICT)。
5. `auth.users` を確認／作成し、`targetUserId` を確定。
6. `users` に upsert:
   - `id = targetUserId`
   - `tenant_id`, `email`, `display_name`, `full_name`, `full_name_kana`, `group_code`, `residence_code`, `language` などを保存／更新。
7. `roles` から `role_key` に対応する `id` を取得し、`user_roles` に登録。
8. `user_tenants` に `(user_id, tenant_id)` を登録。
9. 成功レスポンスを返却。
