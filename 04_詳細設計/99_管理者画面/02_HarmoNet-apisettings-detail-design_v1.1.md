# HarmoNet 詳細設計書: テナント別 API 設定（AI設定画面） v1.1
...
* **認証・認可には共通認証ガード（Middleware + Server Helper）を利用する。**
  * 画面アクセス: Middleware により `tenant_admin` 以外はリダイレクト。
  * API アクセス: `getTenantAdminApiContext` ヘルパーにより権限チェックを行う。
...
* メソッド: `GET`
* パス: `/api/t-admin/ai/settings`

#### 5.1.1 権限チェック

* **`getTenantAdminApiContext` ヘルパーを利用する。**
* セッションの `tenant_id` を取得し、`tenant_admin` ロールを保持しているか確認。
* 権限がない場合は 401/403 エラー。

#### 5.1.2 挙動

1. `tenant_settings` テーブルから `tenant_id` に一致するレコードを取得。
2. レコードが存在しない場合は、デフォルト値を返却（または空の JSON を返却しフロントエンドでデフォルト適用）。
3. レコードが存在する場合は、`config_json.board` 配下の値を返却。
...
1. 入力値のバリデーション。
2. `tenant_settings` テーブルに upsert。
   * `tenant_id` をキーとする。
   * 既存の `config_json` を取得し、`board` 配下のみをマージ更新する（他の設定値を消さないよう注意）。
3. 成功レスポンスを返却。
