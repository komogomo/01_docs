...
- 管理組合ユーザ（viewerRole = `"admin"`）は、`BoardPostFormAuthorSection` で  
  「管理組合として投稿」／「一般利用者として投稿」および「匿名／ニックネーム」の切り替えを行う。
- 一般利用者（viewerRole = `"user"`）にはロール選択は表示しないが、「匿名／ニックネーム」の選択は表示する。
...
- **挙動**:
  - `viewerRole = "admin"` の場合のみロール選択ラジオボタンを表示。
  - 一般利用者（`viewerRole = "user"`）にはロール選択を表示せず、内部的に `"user"` 固定とする。
  - 表示名モード（`displayNameMode`）は初期値未選択とし、明示的な選択を求める。
...
- **許可する拡張子**:
  ...
  - `*.ppt` / `*.pptx`
- **挙動**:
  - ファイル選択コンポーネント（ドラッグ&ドロップ領域含む）を提供し、`accept` 属性に上記拡張子および MIME タイプを指定する。
  - 選択されたファイル一覧を表示し、削除ボタンを提供する。
  - アップロード処理や Storage との連携は共通アップロードコンポーネント側に委譲し、BoardPostForm は「選択されたファイルのメタ情報」を `board_attachments` 登録用 DTO に反映する。
  - 上記以外の拡張子が選択された場合はバリデーションエラーとする。
