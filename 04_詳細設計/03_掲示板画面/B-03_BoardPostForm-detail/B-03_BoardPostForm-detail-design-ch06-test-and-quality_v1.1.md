# B-03 BoardPostForm 詳細設計書 ch06 テスト・品質保証設計 v1.1
...
* 結合テスト（BoardPostForm と投稿/更新 API との連携）
* メッセージ／バリデーション仕様の検証
* ログ出力・フェイルソフト挙動の確認
...
* BoardPostForm から呼び出される投稿 API ラッパ
  ...
  * 例: `upsertBoardPost(mode, postId, request)`
...
1. 画面タイトルが `board.postForm.title`（新規）または `board.postForm.title.edit`（編集）に対応するテキストで表示されること。
2. タイトル／本文／投稿区分／宛先／添付／回覧板のフィールドが表示されていること。
3. **投稿者表示モード（匿名/ニックネーム）選択肢が表示されていること。**
4. 初期状態で:

   * 必須マークが付与されているフィールド（投稿区分・タイトル・本文・**表示名モード**）が期待どおりであること。
   * エラー表示領域が空であること。
   * `isSubmitting = false` 相当の状態で、「投稿する」（または「保存する」）ボタンが有効であること。
...
2. **投稿者表示モード未選択で「投稿する」を押下した場合、**

   * `board.postForm.error.displayName.required` が表示されること。

3. 対象範囲 = GROUP かつグループ未選択で「投稿する」を押下した場合、
...
5. 本文が空、またはエディタのダミー値のみの場合、

   * `board.postForm.error.content.required` または `board.postForm.error.content.invalid` が表示されること。

6. 回覧板設定が不正（回覧板対象外カテゴリに対して回覧 ON など）の場合、

   * `board.postForm.error.circulation.invalid` が表示されること。

7. **添付ファイルバリデーション:**

   * PDF / Office 系ファイル（Word/Excel/PPT）を選択してもエラーにならないこと。
   * 許可外のファイル（例: `.exe`, `.zip`）を選択した場合、`board.postForm.error.attachment.invalidType` が表示されること。
...
1. 入力が有効な状態で「投稿する」を押下した場合、
   ...
   * **確認ダイアログが表示され、以下のプレビュー要素が含まれていること。**
     * 入力したタイトル
     * 選択したカテゴリ
     * 本文（要約）
     * 添付ファイル名
     * 注意喚起メッセージ（全住民に公開されます等）

2. 確認ダイアログで「キャンセル」を押下した場合、

   * ダイアログが閉じ、投稿 API は呼ばれないこと。

3. 確認ダイアログで「はい（投稿する）」を押下した場合、
...
### 6.4.4 編集モード固有の検証（v1.1 新設）

**目的:** 編集モード（`mode='edit'`）における初期化と挙動を確認する。

テストシナリオ:

1. **初期値ロード:**
   * `initialData` (Props) に含まれるタイトル・本文・カテゴリ・添付ファイル・投稿者表示モードが、フォームの初期値として正しくセットされていること。
   * 既存の添付ファイルがリストに表示されていること。

2. **ロール固定:**
   * 管理組合ユーザであっても、編集モード時は「投稿者ロール（管理組合/個人）」の切り替え UI が操作不可（Disabled または非表示）になっていること。

3. **保存動作:**
   * 変更を加えて「保存する」を押下した際、API が `PATCH` メソッドで呼び出されること。
   * 成功時に編集完了トースト（`board.postForm.toast.saveSuccess`）が表示されること。
...
1. フォーム表示時:
   ...
   * context: `{ tenantId, viewerUserId, mode }`
...
* ログ出力・フェイルソフト挙動の検証
* **編集モードおよび追加要件（表示名選択・プレビュー・添付拡張子）の検証**
