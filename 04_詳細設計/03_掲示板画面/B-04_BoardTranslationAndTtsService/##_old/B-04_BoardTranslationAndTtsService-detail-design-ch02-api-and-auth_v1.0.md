# B-04 BoardTranslationAndTtsService 詳細設計書 ch02 API・認証設計 v1.0
...
* Supabase Edge Function からの翻訳キャッシュ生成・削除処理
...
export interface TtsService {
   ...
   * キャッシュ保存やストレージ連携は行わない（本 v1.0 のスコープ外）。
   */
...
* v1.0 では TTS の結果はキャッシュしない前提とし、再生のたびに API を呼び出す。
...
* HarmoNet では、Google Cloud 公式 Node.js SDK（`@google-cloud/translate`）または REST クライアントのどちらかを採用する。

  * v1.0 では SDK 利用を前提とし、REST 直叩きは想定しない。
...
| 翻訳用サービスアカウント JSON | `GOOGLE_APPLICATION_CREDENTIALS` | JSON キーファイルへのパス（Supabase / Vercel の環境変数に設定） |

* 本番環境（Supabase Pro）では、`GOOGLE_APPLICATION_CREDENTIALS` に Supabase 側のファイルパス／シークレットストアパスを設定する。
* 開発環境（ローカルコンテナ）では、Docker コンテナ内のマウントパス、もしくは JSON をそのまま配置したパスを指定する。

パラメータシートには、少なくとも以下を記載する。

* GCP プロジェクト ID
* Translation 用サービスアカウントのメールアドレス
* JSON キーファイルの保管場所（Git 管理外）
...
export class GoogleTranslationService implements TranslationService {
  ...
  async translateOnce(params: TranslateParams): Promise<TranslateResult> {
    // 実装詳細は ch03/ch06 ではなく、Windsurf 実装タスクで具体化する
  }
}
```

* `TranslationServiceClient` は、環境変数から自動的に認証情報を取得する前提とする。
* プロジェクト ID やロケーションは、環境変数または設定定数から参照する（次項参照）。
...
* `mimeType` は本文の保存形式に合わせて決定する。
  ...
  * v1.0 では単純なテキスト前提で `text/plain` を既定とするが、最終決定は B-03 と整合させる。
...
* `BoardPostTranslationService.translateAndCacheForPost` では、少なくとも以下を実装する想定とする。
  ...
  * 単一ターゲット言語での失敗時には、その言語のみスキップし、他言語は継続するかどうかを選択可能な設計とする（v1.0 では「失敗した言語はキャッシュ無し／原文フォールバック」でよい）。
...
* 環境変数: 翻訳 API と同一の `GCP_PROJECT_ID` / `GOOGLE_APPLICATION_CREDENTIALS` を利用する。
...
* v1.0 では、TTS 結果はキャッシュせず、その都度 TTS API に問い合わせて音声を生成する。
...
次章 ch03 では、翻訳キャッシュテーブル `board_post_translations` のスキーマ、および翻訳キャッシュ保有期間の管理方法（tenant_settings との連携、Edge Function バッチ）を詳細に定義する。
