# B-04 BoardTranslationAndTtsService 詳細設計書 ch04 掲示板詳細画面との連携（翻訳・TTS 利用フロー） v1.1
...
> * v1.1 では投稿作成 API の内部で同期的に翻訳を実行する前提で記述するが、将来的に非同期キュー方式へ変更可能な設計とする。
...
4. 再生終了後、ユーザは必要に応じて再度ボタンを押して再生できる。v1.0 時点では TTS 結果はキャッシュせず、その都度 API を呼び出す。
...
* 認証情報（GCP サービスアカウント）は、ch02 で定義した通り、

  * ローカル: `GOOGLE_APPLICATION_CREDENTIALS`（ファイルパス）
  * 本番: `GCP_TRANSLATE_CREDENTIALS_JSON`（JSON シークレット）
    を通じて `TranslationService` / `TtsService` が内部的に利用する。
...
* Edge Function から Google API を呼び出す場合（将来の拡張など）は、Supabase Secrets に格納された `GCP_TRANSLATE_CREDENTIALS_JSON` を利用し、ch02 と同一の認証方式を用いる。

---

本章では、B-04 BoardTranslationAndTtsService と掲示板画面（B-02 / B-03）との連携フローを定義した。
v1.1 では特に、オンライン処理は Next.js Route Handler 経由、本番 Supabase Cloud では GCP 認証情報を Secrets 経由で扱う前提を明示した。
