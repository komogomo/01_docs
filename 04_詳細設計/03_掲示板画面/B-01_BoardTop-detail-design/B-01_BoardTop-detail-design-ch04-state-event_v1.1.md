# B-01 BoardTop 詳細設計書 ch04 状態管理・イベント遷移 v1.1
...
本章では、掲示板TOP画面コンポーネント BoardTop（B-01）における **状態管理** と **ユーザインタラクションイベント** を定義する。対象はフロントエンドの状態のみとし、DB スキーマ（`schema.prisma`）はすでに正として確定している前提で扱う。Windsurf は本章を参照して、React コンポーネント内の `useState` / `useReducer` / カスタムフックの実装を行う。

本 v1.1 では、HOME 画面およびフッターからの遷移仕様に合わせ、BoardTop のフィルタ条件を「カテゴリタブ（タグ）のみ」に整理し、`scope` 関連の状態・イベントを削除した。
...
export type BoardTopState = {
  ...
  viewerRole: "admin" | "user";     // 管理組合 or 一般利用者（将来拡張を見据えた簡易区分）
...
`BoardPostSummary` は ch03 で定義した DTO を参照する。`lastUpdatedAt` は、ユーザに対して「いつの状態か」を明示するための任意項目であり、MVP で不要と判断された場合は削除可能である。
...
HOME 画面やフッターからの遷移時は、リンク側で `tab` を指定することで初期アクティブタブを制御する（例: `/board?tab=notice`）。指定がない場合は `all` とする。
...
export type BoardTopEvent =
  | { type: "INIT"; payload: { tab?: string; page?: string; perPage?: string } }
...
2. `INIT` により URL クエリから `tab`, `page`, `perPage` を解釈し、`BoardTopState` を初期化する。
...
2. 現在の `tab` / `page` / `perPage` を維持したまま、`FETCH_STARTED` → 一覧取得 → `FETCH_SUCCEEDED` or `FETCH_FAILED` を再実行する。
...
4. 将来、検索条件や追加フィルタ（期間、キーワードなど）が増えた場合は、`BoardTopState` / `BoardTopEvent` にフィールドを追加し、本章を v1.2 以降として更新する。既存のフィールドやイベントの破壊的変更は極力避ける。
...
| ---------------------------- | ----- | --------------------------- | ------------------------------------------------ |
| 初回 / 再取得開始 (`FETCH_STARTED`) | INFO  | `"board.top.fetch.start"`   | `{ tab, page, perPage, viewerRole }`             |
| 取得成功 (`FETCH_SUCCEEDED`)     | INFO  | `"board.top.fetch.success"` | `{ tab, page, perPage, viewerRole, totalCount }` |
| 取得失敗 (`FETCH_FAILED`)        | ERROR | `"board.top.fetch.error"`   | `{ tab, page, perPage, viewerRole, errorType }`  |
