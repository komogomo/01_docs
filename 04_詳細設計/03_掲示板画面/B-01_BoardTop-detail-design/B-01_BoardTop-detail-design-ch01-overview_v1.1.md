# B-01 BoardTop 詳細設計書 ch01 概要 v1.1
...
5. 翻訳および音声読み上げに関して、**投稿時に B-04 BoardTranslationAndTtsService により生成された翻訳キャッシュを利用し、一覧上で翻訳済みサマリおよび TTS トリガを提供する**。

   * 現在の UI 言語と異なる言語で投稿された場合、翻訳キャッシュがあれば翻訳済みサマリを表示する。
   * 翻訳キャッシュが存在しない場合に限り、「翻訳」ボタンから投稿単位で翻訳を再実行できるようにする。
   * 読み上げボタンから、翻訳済みサマリまたは原文サマリのいずれかを TTS に渡す。

翻訳キャッシュの生成・更新および TTS の詳細ロジックは B-04 BoardTranslationAndTtsService の責務とし、BoardTop は「キャッシュ済み結果の表示」と「再翻訳・TTS トリガ」を担う。
1 件の投稿に対する全文表示、PDF プレビュー、コメント表示、返信・編集・削除などの操作は BoardDetail（B-02）および BoardPostForm（B-03）の責務とする。
...
* `schema.prisma` に基づく投稿一覧データの入出力 DTO・ソート条件・フィルタ条件・翻訳/TTS 用フラグ（ch03）
* タブ切替・範囲切替・ページング・カードタップ・翻訳ボタン・TTS ボタンなどの状態管理とイベント遷移（ch04）
* 投稿サマリカードの UI 構造、表示項目、アイコン、翻訳/TTS ボタン、静的文言の i18n キー、および空状態・エラー状態のメッセージ仕様（ch05）
* AppHeader / AppFooter / LanguageSwitch / StaticI18nProvider 等の共通部品との結合関係、および HOME / 詳細画面 / 投稿フォームとの遷移関係（ch06）
...
* 掲示板詳細画面 `/board/[id]` の UI・翻訳/TTS ボタン・PDF プレビュー・コメント表示
  → コンポーネント ID: B-02 BoardDetail（別詳細設計書）
* 新規投稿・投稿編集フォームの UI・バリデーション・投稿 API 連携・AI モデレーション
  → コンポーネント ID: B-03 BoardPostForm（別詳細設計書）
* Google Translate / Google Cloud TTS を用いた翻訳・音声読み上げ処理の共通設計
  → B-04 BoardTranslationAndTtsService 詳細設計書、および掲示板基本設計の共通仕様
...
3. DB スキーマ
   ...
   * 翻訳キャッシュは `board_post_translations` テーブル（および必要に応じて TTS キャッシュテーブル）に格納される。
...
| 技術スタック | `harmonet-technical-stack-definition_v4.5.md`    | Next.js / Supabase / Prisma / i18n / OpenAI 等の技術前提。            |
...
| スキーマ   | `schema.prisma`                                  | `board_posts` / `board_post_translations` など掲示板関連テーブル定義（唯一の正）。 |
