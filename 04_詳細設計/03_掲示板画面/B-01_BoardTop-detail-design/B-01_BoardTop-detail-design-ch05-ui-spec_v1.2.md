# B-01 BoardTop 詳細設計書 ch05 UI仕様・メッセージ v1.2
...
本 v1.2 では、HOME 画面およびフッターからの遷移仕様に合わせ、BoardTop のフィルタ要素を **カテゴリタブ（タグ）のみ** に整理し、`scope` タブ関連の UI を削除した。
...
* タブ or セグメントコントロール（3 分割程度）
  ...
  * `board.top.tab.all`    : すべて
  * `board.top.tab.notice` : お知らせ
  * `board.top.tab.rules`  : 運用ルール

HOME 画面およびフッターから遷移した場合の初期アクティブタブは、ch02/ch04 で定義した通りとする。
...
* タブ選択時に `tab` を変更し、一覧取得 API を再呼び出しする。
* 選択中のタブは強調表示し、視覚的に分かるようにする。
* URL クエリ `tab` を更新し、ブックマークや戻る操作で状態が復元できるようにする（詳細は ch04）。
...
新規投稿ボタンの配置位置（上部右寄せボタン / 右下 FAB 等）は ch02 のレイアウト定義と合わせて実装する。
...
翻訳キャッシュ取得エラーは、上記の「一覧取得エラー」とは別扱いとし、原文サマリのみで一覧表示を継続する（ch03 参照）。
...
* 大分類例:
  ...
  * `board.top.tab.*`                : カテゴリタブラベル
  * `board.top.newPost`              : 新規投稿ボタン
  * `board.top.category.*`           : カテゴリバッジ用ラベル
...
本章 v1.2 は、BoardDetail（B-02）および翻訳/TTS サービス（B-04）の仕様に整合する形で、BoardTop における翻訳・TTS・新規投稿 UI を定義している。
今後、カード内に他のアクション（ピン留め、既読マーク等）を追加する場合は、本章を v1.3 以降に更新する。
