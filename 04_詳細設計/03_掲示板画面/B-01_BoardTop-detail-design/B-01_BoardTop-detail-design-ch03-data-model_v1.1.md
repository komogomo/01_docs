# B-01 BoardTop 詳細設計書 ch03 データモデル・入出力仕様 v1.1
...
`contentPreview` と `translatedPreview` は、実装上は同一フィールドに統合してもよいが、
設計上は「原文サマリ」と「翻訳済みサマリ」を明示的に区別できるようにしている。
...
export type BoardTopPageData = {
  ...
  // フィルタ／タブの状態
  scope: "all" | "mine" | "management"; // 表示スコープ（例）

  // 利用者権限
  viewerRole: string;         // 共通ロール定義
  canPost: boolean;           // 新規投稿ボタンを表示できるか
};
```

`scope` の値は、BoardTop ch02 で定義したタブ/フィルタ仕様に従う。
...
export type BoardTopQueryInput = {
  ...
  viewerRole: string;      // 共通ロール定義
  scope: "all" | "mine" | "management"; // 表示スコープ
...
* scope 切替（例）
  ...
  * `scope = "all"`           : 追加条件なし
  * `scope = "mine"`          : `author_id = :userId`
  * `scope = "management"`    : 投稿者ロールが管理組合に該当するレコードのみ
...
本章 v1.1 は、現時点の `schema.prisma` と B-03/B-04 詳細設計、および BoardDetail（B-02）の翻訳/TTS 仕様に整合する形で定義されている。
一覧表示仕様や翻訳/TTS の扱いに変更が入った場合は、本章のバージョンを 1.2 以降に引き上げて差分を明示する。
