# B-01 BoardTop 詳細設計書 ch02 画面構造・コンポーネント構成 v1.1
...
* 役割:
  ...
  * クエリパラメータ（`tab`, `page`, `perPage`）を解釈し、内部状態の初期値を決定する。
...
### 2.2.3 BoardTopControls（BoardTabBar）

* `BoardTopControls` は、BoardTop 上部に配置されるカテゴリタブのコンテナとする。

* `BoardTabBar`

  * タブ: `ALL`, `NOTICE`, `RULES`（掲示板基本設計 ch03 および HOME 画面仕様に準拠）。
  * 選択中タブを視覚的に強調し、タブ切替時に BoardTopPage へイベントを通知する。
  * HOME からの遷移元に応じて、初期状態のアクティブタブは以下とする（詳細は ch06 参照）。

    * HOME「掲示板」タイル / フッター「掲示板」タブ: `ALL`
    * HOME「お知らせ」タイル: `NOTICE`
    * HOME「運用ルール」タイル: `RULES`

### 2.2.4 BoardPostSummarySection

* 一覧表示とページネーションをまとめて扱うセクションコンテナ。次の要素を縦方向に並べる。

1. `BoardPostSummaryList`

   * 一覧データをカード形式で表示する。
   * データが 1 件以上存在する場合のみ表示する。
2. `BoardEmptyState`
...
* 単一の掲示板投稿をカード形式で表示するコンポーネント。含まれる要素（詳細は ch05 で定義予定）:
...
HOME 画面やフッターから遷移する際は、クエリパラメータ `tab` を指定することで初期アクティブタブを制御する（例: `/board?tab=notice`）。

### 2.3.2 クエリパラメータ

BoardTop は、以下のクエリパラメータをサポートする。不正値が指定された場合は、最寄りのデフォルト値へフォールバックする。
...
1. `tab` / `page` / `perPage` を URL から読み込む。
...
* タブバーは横並び配置とし、見出しと一体感のあるデザインとする（具体値は ch05）。
...
* `BoardTopPage`
  ...
  * 子コンポーネントに対して、現在のタブ・ページ情報・投稿一覧・ローディング／エラー状態などを props で渡す。
