# B-02 BoardDetail 詳細設計書 ch07 テスト観点・UT/IT 方針 v1.0
...
* データアクセス:
  ...
  * `fetchBoardDetailPage` / `postBoardComment` など共通データアクセス API は **モック化** し、実際の Supabase 接続は行わない。
...
### 7.4.2 コメント入力・送信（管理組合または投稿可能ロール）

* 条件

  * `viewerRole = "admin"` かつ `canPostComment = true` の状態でレンダリング。
  * コメント投稿 API モックは正常に `BoardCommentItem` を返す。
* 期待値

  * コメント入力欄と「コメントを投稿」ボタンが表示されている。
  * テキスト入力後に送信ボタンをクリックすると、投稿 API が 1 回呼ばれる。
  * 成功後、入力欄が空になり、コメント一覧末尾に新しいコメントが追加される。

### 7.4.3 コメント入力欄の非表示（一般利用者で投稿不可）

* 条件

  * `viewerRole = "user"` かつ `canPostComment = false` の状態でレンダリング。
* 期待値
...
* 条件
  ...
  * コメント投稿 API モックがエラーを throw する。
* 期待値

  * コメント入力欄は残ったまま（ユーザの入力は消さない）。
  * 送信中フラグが解除される（ボタンが再度押下可能な状態）。
  * 「コメントの投稿に失敗しました…」メッセージが表示される。
...
* 観点例:
  ...
  * 純異常系: コメント送信失敗時に `logError("board.detail.comment.error", ...)` が 1 回呼ばれる。
