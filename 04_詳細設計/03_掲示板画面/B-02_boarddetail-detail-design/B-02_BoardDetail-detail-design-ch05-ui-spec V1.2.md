...
* 原文が表示されている状態で、かつ**現在の表示言語が投稿の原文言語と異なる場合**に翻訳ボタンを表示する。
* ボタンラベル: `board.detail.i18n.translate`
* 翻訳中は `board.detail.i18n.translating` としてスピナー付きラベルに切り替え、ボタンを非活性にする。
* 既に翻訳データが存在する場合（自動翻訳済みなど）は、ボタンを非活性状態で表示する（または「翻訳を表示」トグルで切り替え可能とする）。
