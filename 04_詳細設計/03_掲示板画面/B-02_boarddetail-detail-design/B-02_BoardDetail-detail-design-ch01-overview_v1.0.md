# B-02 BoardDetail 詳細設計書 ch01 概要 v1.0
...
2. PDF などの添付ファイルがある場合、添付一覧を表示し、モーダルプレビュー起動の入口を提供する（PDF プレビュー仕様は掲示板共通仕様に従う）。
3. 翻訳機能（Google Translate）と音声読み上げ機能（Google Cloud TTS）を呼び出す UI を提供し、翻訳済みテキストおよび TTS 再生の状態を表示する（共通翻訳/TTS コンポーネントとの結合）。
4. コメント機能が有効な場合、コメント一覧の表示およびコメント投稿 UI を提供する（MVP の範囲は掲示板基本設計および決定サマリに従う）。
...
* BoardTop / PostForm / 共通翻訳/TTS / Logger との結合関係・役割分担（ch06）
