# B-02 BoardDetail 詳細設計書 ch02 画面構造・コンポーネント構成 v1.0
...
BoardDetail は、HarmoNet 共通レイアウト（AppHeader / AppFooter / LanguageSwitch / StaticI18nProvider）配下の「メインコンテンツ領域」に表示される。
...
PDF プレビューは掲示板共通仕様（モーダルで表示、背景タップ無効、下部に「タップで閉じる」）に従い、BoardDetail ではそのモーダルを起動するトリガと閉じる制御のみを行う。
...
* コメント機能の有効/無効、投稿権限（管理組合のみ／全利用者など）は掲示板基本設計に従う。
* BoardDetail 側では、

  * コメントリスト
  * コメント入力欄（必要なロールのみ）
  * 投稿ボタン
    の構造を定義し、コメント送信イベントをデータアクセス層に引き渡す。
...
初版では、BoardDetail 独自のクエリパラメータは定義しない。
PDF プレビュー状態などの一時的な UI 状態は URL には乗せず、画面内部の state として管理する。

将来的に、コメント一覧のソート順やページングなどを URL で表現する必要が出てきた場合は、その時点でクエリパラメータを設計し、本章を v1.1 以降に更新する。
