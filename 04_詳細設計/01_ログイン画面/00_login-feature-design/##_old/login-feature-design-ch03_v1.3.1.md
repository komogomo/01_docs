HarmoNet ログイン画面 詳細設計書 – 第3章 コールバック処理・セッション確立仕様 v1.3.1
| 属性                     | 値                                                 |
| ---------------------- | ------------------------------------------------- |
...
| **Supersedes**         | v1.3                                              |
| **Linked Index**       | `login-feature-design-ch00-index_v1.3.1.md`       |
...
tenant_id識別フロー詳細（Phase9制約）
1.前提：users(email UNIQUE, tenant_id UUID NOT NULL) — 1ユーザー＝1テナント固定。
2.Magic Link送信時：users.email の存在のみ検証（存在有無は秘匿）。
3.コールバック時：exchangeCodeForSession() 成功後、users テーブルを email で検索し、該当レコードの tenant_id を取得。
4.WT拡張：取得した tenant_id を Supabase セッションのクレームに付与し、RLS 参照対象とする。
5.失敗時：tenant_id 未割当 → /login?error=invalid_tenant、DB障害 → /login?error=network。
6.将来拡張：複数テナント所属（例：管理会社アカウント）は Phase10 以降で設計予定。
